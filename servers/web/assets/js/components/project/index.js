  ...
  .factory("project", ["$http", "$q", "DirectoryEntry", "TextEntry", "visitor", "commander", "config", function($http, $q, DirectoryEntry, TextEntry, visitor, commander, config) {
    ...
    Project.prototype.clearTree = function() {
      ...
      this.traverse(function(entry) {
        queue = queue.then(function() {
          if (entry.isFile()) return commander.execute("file.remove", {
            parent: entry.parent,
            file: entry
          });
          else return commander.execute("directory.remove", {
            parent: entry.parent,
            directory: entry
          });
        });
    ...
    Project.prototype.destroy = function(plunkId) {
      ...
      return visitor.getCredentials()
        ...
        .then(function(credentials) {
          return $http.delete(config.url.api + "/plunks/" + plunkId, {
            params: {
              token: credentials
            }
          });
    ...
    Project.prototype.fork = function() {
      ...
      return visitor.getCredentials().then(function(credentials) {
        return $http.post(config.url.api + "/plunks/" + self.plunk.id + "/forks", payload, {
          params: {
            token: credentials
          }
        }).then(function(response) {
    ...
    Project.prototype.setTree = function(tree) {
      ...
      return this.clearTree().then(function() {
        ...
        function directoryCreate(parent, entry) {
          return commander.execute("directory.create", {
            parent: parent,
        ...
        function fileCreate(parent, entry) {
          return commander.execute("file.create", {
            parent: parent,
            filename: entry.filename,
            active: entry.active
          }).then(function(file) {
            return commander.execute("text.insert", {
              path: file.getPath(),
              offset: 0,
              text: entry.contents
            });
    ...
    Project.prototype.openTree = function(tree) {
      ...
      var returnPromise = commander.execute("project.setTree", {
        tree: tree
      }).then(function() {
        self.tree = sha;
      });
      ...
      returnPromise = returnPromise.then(function() {
        _.each(self.entries, function(entry) {
          if (entry.active) {
            return commander.execute("workspace.open", {
              type: 'code',
              id: entry.entryId,
              blank: true
            });
    ...
    Project.prototype.open = function(plunkId) {
      ...
      return visitor.getCredentials().then(function(credentials) {
        return $http.get(config.url.api + "/plunks/" + plunkId, {
          params: {
            token: credentials
          }
        }).then(function(response) {
    ...
    Project.prototype.remove = function(parent, entry, filename) {
      ...
      if (entry.children) {
        _.forEach(entry.children, function(child) {
          if (child.isDirectory()) commander.execute("directory.remove", {
            parent: entry,
            directory: child
          });
          else if (child.isFile()) commander.execute("file.remove", {
            parent: entry,
            file: child
          });
        });
    ...
    Project.prototype.save = function(user) {
      ...
      return $http.post('/saveFile', payload).then(function(response) {
        self.plunk = response.data;
    ...
    Project.prototype.getEntry = function(path) {
      var test = _.isRegExp(path) ? path.test.bind(path) : function(cmp) {
        return cmp === path;
      };
        ...
        return $modal.open({
          ...
          controller: ["$scope", "$modalInstance", function($scope, $modalInstance) {
            ...
            $scope.refreshFavorites = function() {
              if (visitor.isMember()) {
                return visitor.getCredentials().then(function(credentials) {
                  return $http.get(config.url.api + "/plunks", {
                    params: {
      ...
      handler: ["$modal", function($modal) {
        ...
        return $modal.open({
          ...
          if (updated.readme !== editing.readme) {
            var readmePromise = readme ? $q.when(readme) : commander.execute("file.create", {
              filename: "README.md"
            });

            return readmePromise.then(function(readme) {
              return commander.execute("text.remove", {
                path: readme.getPath(),
                offset: 0,
    ...
    commander.addCommand({
      ...
      handler: ["$modal", function($modal) {
        return $modal.open({
          ...
          return commander.execute("project.publish", {
